...
DemoWeave adds deterministic execution, evidence, freshness, and review boundaries:

```text
understand → run → capture evidence → render when needed → check freshness → write → preview diff → reviewed apply
```
...
| Evidence and provenance | Available | `TerminalTrack v1`, screenshot `Evidence v1`, `Manifest v1`, source/Flow/artifact fingerprints, and derivation relationships |
...
| Browser recording / composed tutorials | Planned | Screenshot Evidence exists; recording/composition remains later roadmap work |

Interactive PTY input, browser video/GIF recording, native desktop/mobile automation, and complete video/tutorial generation are outside the current implementation.
...
Requirements: Node.js 20+ and pnpm. FFmpeg is optional for terminal GIF rendering. Playwright Chromium is required only when executing web Flows.
...
On Linux, Playwright may also need its system browser dependencies; `playwright install --with-deps chromium` installs those on supported distributions. `doctor` reports whether Chromium is ready.
...
- **Evidence** — a typed artifact record with lifecycle, producer, provenance, source/artifact fingerprints, and optional derivation metadata. Current captures include terminal tracks and web PNG screenshots.
